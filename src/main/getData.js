...
let apiDomain = 'https://public-operation-common.mihoyo.com'
const apiPath = '/common/gacha_record/api/getGachaLog'
...
const detectGameType = async (userPath) => {
  ...
  try {
    await fs.access(path.join(userPath, '/AppData/LocalLow/miHoYo/', '绝区零/output_log.txt'), fs.constants.F_OK)
    list.push('绝区零')
  } catch (e) {}
  try {
    await fs.access(path.join(userPath, '/AppData/LocalLow/miHoYo/', 'Zenless Zone Zero/output_log.txt'), fs.constants.F_OK)
    list.push('Zenless Zone Zero')
  } catch (e) {}
  if (config.logType) {
    if (config.logType === 2) {
      list.reverse()
...
const readLog = async () => {
  ...
  try {
    ...
    const promises = gameNames.map(async name => {
      ...
      const gamePathMch = logText.match(/\w:[/\\].+(ZenlessZoneZero_Data)/)
      if (gamePathMch) {
        const [cacheText, cacheFile] = await getCacheText(gamePathMch[0])
        const urlMch = cacheText.match(/https.+?auth_appid=webview_gacha.+?authkey=.+?game_biz=nap_\w+/g)
        if (urlMch) {
...
const getGachaLog = async ({ key, page, name, retryCount, url, endId }) => {
  const text = i18n.log
  try {
    const res = await request(`${url}&gacha_type=${key}&page=${page}&size=${20}&real_gacha_type=${key}${endId ? '&end_id=' + endId : ''}`)
...
const tryGetUid = async (queryString) => {
  ...
  try {
    for (let [key] of defaultTypeMap) {
      const res = await request(`${url}&gacha_type=${key}&page=1&size=6&real_gacha_type=${key}`)
      checkResStatus(res)
...
const getQuerystring = (url) => {
  ...
  if (host.includes('hoyoverse.com') || host.includes('nap-api-os')) {
    apiDomain = 'https://public-operation-nap-sg.hoyoverse.com'
  } else {
    apiDomain = 'https://public-operation-common.mihoyo.com'
  }
...
const tryRequest = async (url, retry = false) => {
  ...
  const testUrl = `${apiDomain}${apiPath}?${queryString}&gacha_type=1&page=1&size=1&real_gacha_type=1`
...
const fetchData = async (urlOverride) => {
  ...
  const res = await request(`${apiDomain}${apiPath}?${queryString}&gacha_type=1&page=1&size=1&real_gacha_type=1`)
  const regionTimeZone = res.data.region_time_zone || 0
  const data = { result, time: Date.now(), typeMap, uid: originUid, lang, region_time_zone: regionTimeZone }
...
const convertTimeZone = (timeStr, fromZone, toZone) => {
  if (fromZone === toZone) return timeStr
  const date = new Date(timeStr)
  if (isNaN(date.getTime())) return timeStr
  const diff = (toZone - fromZone) * 60 * 60 * 1000
