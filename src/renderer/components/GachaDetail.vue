<template>
  ...
  <p class="text-gray-600 text-xs mb-1">
    <span :title="`${text.character}${colon}${detail.count5c}\n${text.weapon}${colon}${detail.count5w}\n${text.bangboo}${colon}${detail.count5b}`" class="mr-3 whitespace-pre cursor-help text-yellow-500">
      <span class="min-w-10 inline-block">{{text.star5}}{{colon}}{{detail.count5}}</span>
      [{{percent(detail.count5, detail.total)}}]
    </span>
    <br><span :title="`${text.character}${colon}${detail.count4c}\n${text.weapon}${colon}${detail.count4w}\n${text.bangboo}${colon}${detail.count4b}`" class="mr-3 whitespace-pre cursor-help text-purple-600">
      <span class="min-w-10 inline-block">{{text.star4}}{{colon}}{{detail.count4}}</span>
      [{{percent(detail.count4, detail.total)}}]
    </span>
    <br><span :title="`${text.weapon}${colon}${detail.count3w}\n${text.bangboo}${colon}${detail.count3b}`" class="text-blue-500 whitespace-pre cursor-help">
  ...
  <p class="text-gray-600 text-xs mb-1">
    {{text.historyS}}{{colon}}
    <span v-if="!detail.ssrPos.length" class="text-gray-400">-</span>
  ...
  <p class="text-gray-600 text-xs">{{text.averageS}}{{colon}}<span class="text-green-600" v-if="detail.ssrPos.length">{{avg5(detail.ssrPos)}}</span><span v-else class="text-gray-400">-</span></p>
</template>
