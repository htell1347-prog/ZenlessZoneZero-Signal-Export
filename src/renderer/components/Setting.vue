<template>
  <div class="bg-white pt-2 pb-4 px-6 w-full h-full absolute inset-0">
    ...
    <p class="text-gray-600 text-xs mt-1 pb-6">Github: <a @click="openGithub" class="cursor-pointer text-blue-400">https://github.com/htell1347-prog/ZenlessZoneZero-Signal-Export</a></p>
  </div>
</template>
...
const saveSetting = async () => {
  const keys = ['lang', 'logType', 'proxyMode', 'autoUpdate', 'fetchFullHistory', 'gistsToken', 'readableJSON']
  for (let key of keys) {
...
const openGithub = () => shell.openExternal('https://github.com/htell1347-prog/ZenlessZoneZero-Signal-Export')
const openLink = (link) => shell.openExternal(link)

const exportUIGFJSON = async () => {
  data.loadingOfUIGFJSON = true
  try {
    await ipcRenderer.invoke('EXPORT_UIGF_JSON', settingForm.uigfAllAccounts)
  } catch (e) {
    ElMessage({
      message: e.message || e,
      type: 'error'
